...
from graph_factory_interface import GraphFactoryInference
...
def new_build_train_dattasets(config):
...
def new_build_testing_datasets(config):
...
def build_testing_generator(config, csv_path, data_fdict):
...
def get_graph(batch):
    if len(batch) != 2:
        ...
        from_idx_list = torch.from_numpy(graph.from_idx_list).long()
        to_idx_list = torch.from_numpy(graph.to_idx_list).long()
        graph_ad = torch.from_numpy(graph.graph_ad)
        edge_idx = torch.from_numpy(graph.edge_idx).long()

        return node_features, edge_features, from_idx, to_idx, graph_idx, from_idx_list, to_idx_list, graph_ad, edge_idx
    else:
        graph, labels = batch
        node_features = torch.from_numpy(graph.node_features)
        edge_features = torch.from_numpy(graph.edge_features)
        ...
        return node_features, edge_features, from_idx, to_idx, graph_idx, from_idx_list, to_idx_list, graph_ad, edge_idx, labels

def get_embedding_graph(batch):
    graph, labels = batch
    node_features = torch.from_numpy(graph.node_features)
    edge_features = torch.from_numpy(graph.edge_features)
    from_idx = torch.from_numpy(graph.from_idx).long()
