...
def compute_self_attention(node_states, from_idx, to_idx):


    self_att_mat_ori = torch.mm(node_states, torch.transpose(node_states, 1, 0))
    self_attention_mat = torch.zeros_like(self_att_mat_ori)
    self_attention_mat[to_idx, from_idx] = self_att_mat_ori[to_idx, from_idx]
    soft_mat = torch.softmax(self_attention_mat, dim=1)
    values = soft_mat[to_idx, from_idx]

    return values
    # self_attention_mat = torch.mm(node_states, torch.transpose(node_states, 1, 0))
    # se_tt = torch.zeros_like(self_attention_mat)
    # se_tt[from_idx, to_idx] = self_attention_mat[from_idx, to_idx]
    #
    # results = []
    #
    # for i in range(len(graph_ad)):
    #
    #     soft_mat = torch.softmax(self_attention_mat[graph_ad[i][0]:graph_ad[i][1]+1, graph_ad[i][0]:graph_ad[i][1]+1], dim=1)
    #
    #     from_list = from_idx_list[edge_idx == i]
    #     to_list = to_idx_list[edge_idx == i]
    #
    #     values = soft_mat[to_list,from_list]
    #
...
def graph_embedding_prop_once(node_states,
                              from_idx,
                              to_idx,
                              message_net,
...
def graph_prop_once(node_states,
    ...
    ce = compute_self_attention(node_states, from_idx, to_idx)
    messages = torch.mul(ce.reshape(messages.shape[0],1), messages)
...
class GraphPropLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if self._layer_norm:
            self.layer_norm1 = nn.LayerNorm(self._node_state_dim * 2,float(1e-05))
    ...
    def _compute_aggregated_messages(
            self, node_states, from_idx, to_idx, graph_idx, n_graphs, from_idx_list, to_idx_list, graph_ad, edge_idx, edge_features=None):
        ...
        # attention1, attention2 = compute_self_attention(node_states, graph_dix, n_graphs)

        # delete unsorted_segment_sum
        ...
        # aggregated_messages = compute_self_attention(node_states, from_idx, to_idx, graph_dix, n_graphs, aggregated_messages)
        # optionally compute message vectors in the reverse direction
        if self._use_reverse_direction:
            ...
            # reverse_aggregated_messages = compute_self_attention(node_states, to_idx, from_idx, graph_dix, n_graphs, reverse_aggregated_messages)
        ...
        if self._layer_norm:
            ...
            # aggregated_messages = nn.LayerNorm(aggregated_messages)


        return aggregated_messages

    def _compute_embedding_aggregated_messages(
        ...
        aggregated_messages = graph_embedding_prop_once(
            node_states,
            from_idx,
            to_idx,
            self._message_net,
        ...
        if self._use_reverse_direction:
            reverse_aggregated_messages = graph_embedding_prop_once(
                node_states,
                to_idx,
                from_idx,
                self._reverse_message_net,
    ...
    def forward(self,
        ...
        """
        aggregated_messages = self._compute_embedding_aggregated_messages(
...
class GraphEmbeddingNet(nn.Module):
    ...
    def __init__(self,
        ...
        self._reverse_dir_param_different = reverse_dir_param_different
        self._layer_norm = layer_norm
        self._layers_accumulate = layers_accumulate
        self._prop_layers = []
    ...
    def forward(self,
        ...
        # if(self._layers_accumulate):
        #     stacked_tensor = torch.stack(layer_outputs)
        #
        #     node_states = torch.sum(stacked_tensor, dim=0)

        graph_vectors = self._aggregator(node_states, graph_idx, n_graphs)

        return graph_vectors, node_states
