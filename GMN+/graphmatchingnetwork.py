...
def batch_self_attention(data,
    ...
    self_attention_mat = compute_cross_attention(data,data.T,sim)
...
class GraphPropMatchingLayer(GraphPropLayer):
    ...
    def forward(self,
        ...
        aggregated_messages = self._compute_aggregated_messages(
            node_states, from_idx, to_idx,  graph_idx, n_graphs, from_idx_list, to_idx_list, graph_ad, edge_idx,edge_features=edge_features)

        # self_attention_messages =
        cross_graph_attention = batch_block_pair_attention(
            node_states, graph_idx, n_graphs, similarity=similarity)
        attention_input = (node_states - cross_graph_attention) / 0.8
        # print("aten0.5")
...
class GraphMatchingNet(GraphEmbeddingNet):
    ...
    def __init__(self,
        ...
        super(GraphMatchingNet, self).__init__(
            ...
            node_hidden_sizes,
            n_prop_layers,
            layers_accumulate = layers_accumulate,
    ...
    def _apply_layer(self,
        ...
        return layer(node_states, from_idx, to_idx, graph_idx, n_graphs,from_idx_list, to_idx_list, graph_ad, edge_idx,
